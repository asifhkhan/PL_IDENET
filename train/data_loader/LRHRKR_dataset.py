...
class LRHRDataset(Dataset):
    ...
    def __init__(self, dataroot, is_train, scale, patch_size, rgb_range, noise_std):
        ...
        if self.train:
            self.dataroot_hr = dataroot+'DF2K/train/HR/'
            self.dataroot_lr = dataroot+'DF2K/train/LR/'
            self.dataroot_kr = dataroot + 'DF2K/train/KERNEL/'
    ...
    def __getitem__(self, idx):
        lr, hr, lr_path, hr_path, kr, kr_path = self._load_file(idx)
        
        if self.train:
            lr, hr, sigma= self._get_patch(lr, hr)
            # lr, hr, sigma, sobel_max = self._get_patch(lr, hr)
            # os.makedirs('temp', exist_ok=True)
            # cv2.imwrite(os.path.join('temp', str(sobel_max))+'.png',hr)
        else:
            lr, hr, sigma = self._get_patch(lr, hr)
            # lr, hr, sigma = self._get_test(lr, hr)
            
        lr_tensor, hr_tensor, kr_tensor = common.np2Tensor([lr, hr, kr], self.rgb_range)
        kr_tensor = kr_tensor/255.
        sigma_tensor = torch.tensor(sigma)
        return {'LR': lr_tensor, 'HR': hr_tensor, 'sigma':sigma_tensor, 'LR_path': lr_path, 'HR_path': hr_path, 'KR_tensor': kr_tensor, 'KR_path':kr_path}
    ...
    def _load_file(self, idx):
        ...
        kr_path = self.paths_KR[idx]
        lr = common.read_img(lr_path, 'img')
        hr = common.read_img(hr_path, 'img')
        kr = common.read_img_grey(kr_path, 'img')

        return lr, hr, lr_path, hr_path, kr, kr_path
