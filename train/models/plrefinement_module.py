...
class PromptInteractionBlock(nn.Module):
    def __init__(self, dim=192, num_heads=1, ffn_expansion_factor=2.66, bias=False, LayerNorm_type='WithBias'):
        ...
        #self.conv_1 = nn.Conv2d(dim, 96, kernel_size=1, bias=bias)
...
class BasicLayer(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
        ...
        x = count_parameters(self.blocks)
        print('stl', x)
...
class RSTB(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
        ...
        x = count_parameters(self.pgm)
        print('pgm', x)
        x2 = count_parameters(self.pim)
        print('pim', x2)
        # self.pim = PromptInteractionBlock()
        # self.conv_2 = nn.Conv2d(dim, dim*prompt_scale_factor, kernel_size=1, bias=bias)
...
class SwinIR(nn.Module):
    ...
    def __init__(self, img_size=64, patch_size=1, in_chans=3,
        ...
        x = count_parameters(self.conv_first )
        print('conv1', x)
        ...
        if resi_connection == '1conv':
            ...
            x = count_parameters(self.conv_after_body)
            print('convaftebody', x)
        elif resi_connection == '3conv':
            # to save parameters and memory
        ...
        if self.upsampler == 'pixelshuffle':
            ...
            x = count_parameters(self.conv_last)
            print('convlast', x)
...
if __name__ == '__main__':
    model = SwinTransformerBlock(dim=96, input_resolution=(32,32), num_heads=6)
    x = count_parameters(model)
    print('tb', x)
    # upscale = 4
    # window_size = 8
    # height = 32  # (1024 // upscale // window_size + 1) * window_size
    # width = 32  # (720 // upscale // window_size + 1) * window_size
