...
def p2o(psf, shape):
    ...
    otf = torch.view_as_real(torch.fft.fft(otf, dim=2))
...
class DataNet(nn.Module):
    ...
    def forward(self, x, FB, FBC, F2B, FBFy, alpha, sf):
        ...
        FR = FBFy + torch.view_as_real(torch.fft.fft(alpha*x, dim=2))
        x1 = cmul(FB, FR)
        ...
        # Xest = torch.view_as_real(torch.fft.irfft(FX, dim=2))
        # Xest = torch.fft.irfft(FX, 2)
        Xest =  torch.fft.irfft(torch.view_as_complex(FX), n=FX.shape[2], dim=2)
...
class FFT(nn.Module):
    ...
    def forward(self, y, x, k, sf):
        ...
        F2B = r2c(cabs2(FB))
        STy = upsample(y, sf=sf)
        # FBFy = cmul(FBC, torch.fft.rfft(STy, 2))
        FBFy = cmul(FBC,torch.view_as_real(torch.fft.fft(STy, dim=2)))
        ...
        lambda_par = 0.02
        x = self.d(x, FB, FBC, F2B, FBFy, lambda_par, sf)
