...
import torch
import models.median_pool as median_pool
...
def get_uperleft_denominator(img, kernel):
    ker_f = convert_psf2otf(kernel, img.size()) #[1, 1, 530, 370, 2] # discrete fourier transform of kernel
    nsr = wiener_filter_para(img) #1, 1, ,1 ,1
    denominator = inv_fft_kernel_est(ker_f, nsr )# 1,1,530,370,2
    # img1 = img.cuda() #1 ,1, 530, 370

    # numerator = torch.rfft(img1, 3, onesided=False) #1, 1, 530, 370, 2
    numerator = torch.fft.fftn(img, dim=(2, 3))
    numerator = torch.view_as_real(numerator)
    deblur = deconv(denominator, numerator) #1, 1, 530, 370
    return deblur
...
def inv_fft_kernel_est(ker_f, NSR):
    inv_denominator = ker_f[:, :, :, :, 0] * ker_f[:, :, :, :, 0] \
                      + ker_f[:, :, :, :, 1] * ker_f[:, :, :, :, 1] + NSR #1, 1, 530, 370
    # pseudo inverse kernel in flourier domain.
    inv_ker_f = torch.zeros_like(ker_f) #1, 1, 530, 370,2
...
def deconv(inv_ker_f, fft_input_blur):
    ...
    deblur = torch.view_as_complex(deblur_f)
    deblur = torch.fft.ifftn(deblur, dim = (2,3)).real
